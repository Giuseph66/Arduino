...
void prepararCafeInstantaneo(String quantity, String encryptedKey, String nome);
// Protótipo decifrar chave
...
// === Função utilitária: decifrar chave (Base64) ===
String decryptKey(String encryptedKey) {
  size_t len = encryptedKey.length();
  if (len == 0) return "";
  size_t outLen = 0;
  unsigned char *output = (unsigned char*) malloc(len);
  if (!output) return "";
  int ret = mbedtls_base64_decode(output, len, &outLen, (const unsigned char*) encryptedKey.c_str(), len);
  if (ret != 0) {
    free(output);
...
void prepararCafeInstantaneo(String quantity, String encryptedKey, String nome) {
  // Aguarda se a máquina estiver ocupada
  if (generatedSequence.length() > 0) {
  while (isBusy()) {
    // Permite outras tarefas enquanto espera
    webSocket.loop();
    server.handleClient();
    atualizarScroll();
    delay(50);
  ...
  // Decifra a chave criptografada (Base64) e registra
  String decrypted = decryptKey(encryptedKey);
  Serial.println("Chave recebida (enc): " + encryptedKey);
  Serial.println("Chave decifrada: " + decrypted);
  if (decrypted != "1024") {
    lcd.clear();
    lcd.setCursor(0, 0);
    ...
    case WStype_TEXT:
      ...
      if (cmd == "piscar") {
        ...
        cancelarSequenciaWS();
          ...
          // Formato esperado: instant|qtd|chave|nome (instant|1/4|MINHA_CHAVE|Joao)
          int p1 = cmd.indexOf('|');
          int p2 = cmd.indexOf('|', p1 + 1);
          int p3 = cmd.indexOf('|', p2 + 1);
          if (p1 > 0 && p2 > p1 && p3 > p2) {
            String qty = cmd.substring(p1 + 1, p2);
            String chave = cmd.substring(p2 + 1, p3);
            String nome = cmd.substring(p3 + 1);
            prepararCafeInstantaneo(qty, chave, nome);
          } else {
            webSocket.sendTXT("erro:formato");
          }
