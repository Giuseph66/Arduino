...
// === Token "crip" v2 ===
const uint64_t K1 = 73856093ULL;
const uint64_t K2 = 83492791ULL;
...
// Novo protótipo: café instantâneo
void prepararCafeInstantaneo(String quantity, String token, String nome);
// Protótipo verificar token crip
bool verificarTokenCrip(String tokenRx);
...
void validarSequenciaWS(String seq, String quantity , String user) {
  ...
  if (seq == generatedSequence) {
    ...
    if (user == "") {
      ...
      iniciarScroll("Calma "+ user +", ja vai ficar pronto!");
    }
  } else {
    lcd.clear();
    atualizarRelogio(true);
...
// === Função utilitária: verificar token "crip" ===
bool verificarTokenCrip(String tokenRx) {
  time_t now = time(nullptr);
  long minutoAtual = (now / 60) * 60; // arredondado

  uint64_t tokenCalc = ((minutoAtual * K1) ^ K2) % 1000000;
  uint64_t tokenPrev = (((minutoAtual - 60) * K1) ^ K2) % 1000000;
  uint64_t tokenNext = (((minutoAtual + 60) * K1) ^ K2) % 1000000;

  Serial.printf("minuto=%ld tokenCalc=%llu tokenPrev=%llu tokenNext=%llu tokenRx=%s\n",
                minutoAtual, (unsigned long long)tokenCalc, (unsigned long long)tokenPrev, (unsigned long long)tokenNext, tokenRx.c_str());

  uint64_t tokenNum = (uint64_t)tokenRx.toInt();
  return tokenNum == tokenCalc || tokenNum == tokenPrev || tokenNum == tokenNext;
}

// === Café instantâneo (sem sequência) ===
void prepararCafeInstantaneo(String quantity, String token, String nome) {
  ...
  // Verifica o token crip
  Serial.println("Token recebido: " + token);
  if (!verificarTokenCrip(token)) {
    lcd.clear();
    atualizarRelogio(true);
    lcd.setCursor(0, 0);
    ...
    delay(2000);
    lcd.noBacklight();
    webSocket.sendTXT("erro:token_invalido");
    return;
  }
  
  // Configura usuário atual
  user = nome;
  newUser = nome;

  // Limpa qualquer sequência pendente
  generatedSequence = "instant";
  countdownActive = false;

  // Aciona servo conforme quantidade
  lcd.clear();
  atualizarRelogio(true);
  lcd.backlight();
  lcd.setCursor(0, 0);
  ...
  if (nome == "") {
    ...
    iniciarScroll("Calma "+ nome +", ja vai ficar pronto!");
  }

  webSocket.sendTXT("ok:cafe_instantaneo");
}
    ...
    case WStype_TEXT:
      ...
      if (cmd == "piscar") {
        ...
        ESP.restart();
          ...
          String token = cmd.substring(p2 + 1);
          newUser = cmd.substring(p1 + 1, p2);
        ...
        Serial.println("validar|" + cmd);
          ...
          if (p1 > 0 && p2 > p1) {
            ...
            validarSequenciaWS(seq, qty, newUser);
          } else {
            webSocket.sendTXT("erro:formato");
          }
      }else if (cmd == "cancelar"){
        cancelarSequenciaWS();
      } else if (cmd.startsWith("instant|") || cmd.startsWith("coffee|")) {
          // Formato esperado: instant|qtd|token|nome (instant|1/4|123456|Joao)
          int p1 = cmd.indexOf('|');
          int p2 = cmd.indexOf('|', p1 + 1);
          int p3 = cmd.indexOf('|', p2 + 1);
          if (p1 > 0 && p2 > p1 && p3 > p2) {
            String qty = cmd.substring(p1 + 1, p2);
            String token = cmd.substring(p2 + 1, p3);
            String nome = cmd.substring(p3 + 1);
            prepararCafeInstantaneo(qty, token, nome);
          } else {
            webSocket.sendTXT("erro:formato");
          }
...
void atualizarRelogio(bool force) {
  ...
  // força repintar se estamos no primeiro draw ou se mudamos de minuto
  int minutoAbsoluto = (tmNow->tm_hour - 1) * 60 + tmNow->tm_min; // Adiciona 1 hora
  if (!force && minutoAbsoluto == lastClockMinute) return;
  lastClockMinute = minutoAbsoluto;

  char buf[6];
  sprintf(buf, "%02d:%02d", tmNow->tm_hour - 1, tmNow->tm_min); // Adiciona 1 hora
  lcd.setCursor(11, 1);          // 5 últimas colunas da 2ª linha
  lcd.print(buf);
}
...
void setup() {
  ...
  // Configura NTP para sincronizar relógio (necessário para TOTP)
  configTime(-3 * 3600, 0, "pool.ntp.org", "time.nist.gov"); // UTC-3 (Brasil)
  lcd.clear();
  atualizarRelogio(true);
  lcd.setCursor(0, 0);
  lcd.print("Sincronizando relógio...");
  lcd.setCursor(0, 1);
  lcd.print("Aguarde...");
  Serial.println("Sincronizando relógio...");
  time_t now = time(nullptr);
  while (now < 8 * 3600 * 2) {  // espera até ter hora válida
    delay(500);
    now = time(nullptr);
  }
  lcd.clear();
  atualizarRelogio(true);
  lcd.setCursor(0, 0);
  lcd.print("Relógio sincronizado!");
  atualizarRelogio(true);
  Serial.println("Relógio sincronizado!");

  Serial.println("Sistema de token 'crip' inicializado!");
